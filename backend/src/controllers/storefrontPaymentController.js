...
const buildVariantInfo = (item) => {
  ...
  if (item.item_data) {
    try {
      ...
      // Invalid item_data ignore
    }
  }

  /*
   * Order snapshot/debug purpose.
   */
  variantInfo.sale_mode =
    item.sale_mode || "piece";

  variantInfo.unit_name =
    item.unit_name ||
    (item.sale_mode === "meter"
      ? "meter"
      : "piece");
...
const isValidQuantityStep = (
  ...
  const minimum = roundQuantity(minimumQuantity);
  const step = roundQuantity(quantityStep);
...
const prepareCheckoutTotals = async (req, connection) => {
  ...
  const minimumQuantity =
    saleMode === "meter"
      ? Number(item.minimum_quantity || 1)
      : 1;

  const quantityStep =
    saleMode === "meter"
      ? Number(item.quantity_step || 0.5)
      : 1;
  ...
  if (
    ...
    return {
      error:
        saleMode === "meter"
          ? `${item.name} must be ordered from ${minimumQuantity} ${unitName} in steps of ${quantityStep} ${unitName}`
          : `Invalid quantity for ${item.name}`,
    };
  }

  /*
   * Piece/size products decimal quantity
   * accept cheyyakudadhu.
   */
  if (
    saleMode !== "meter" &&
    ...
    return {
      error: `${item.name} quantity must be a whole number`,
    };
  }

  const unitPrice = Number(
    item.item_price ||
      item.variant_offer_price ||
      item.variant_price ||
      item.offer_price ||
      item.price ||
      0
...
/*
 * Order place ayye exact moment lo
 * current store shipping setting ni snapshot chesthundi.
 */
    ...
    return successResponse(
      res,
      {
        order: {
          ...
          estimated_delivery_days:
  savedOrder.estimated_delivery_days,

estimated_delivery:
  savedOrder.estimated_delivery,

courier_estimated_delivery:
  savedOrder.courier_estimated_delivery,
